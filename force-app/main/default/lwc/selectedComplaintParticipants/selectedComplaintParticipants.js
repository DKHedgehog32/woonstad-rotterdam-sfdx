...
export default class SelectedComplaintParticipants extends LightningElement {
    ...
    @wire(getPicklistValues, {
        recordTypeId: '01206000001AUKqAAA', fieldApiName: 'ComplaintParticipant.Role'
    })
    ...
    @wire(getPicklistValues, {
        recordTypeId: '01206000001AUKqAAA', fieldApiName: 'ComplaintParticipant.Status'
    })
    ...
    set recorddata(recorddata){
        for (let [key, value] of  recorddata.entries()) {
            ...
            if(this._recorddata.has(key)) {
                console.log('##SelectedComplaintParticipants: already included record with id: ' + key);
            }
            else {
        ...
        for (let [key, value] of this._recorddata.entries()) {
            ...
            if(this._selectedData.has(key)){
                console.log("##SelectedComplaintParticipants: key Already present in this._selectedData: " + key);
            }
            else {
        ...
        if (this.recorddataarr.length == 0) {
            console.log('##SelectedComplaintParticipants: Found zero record');
